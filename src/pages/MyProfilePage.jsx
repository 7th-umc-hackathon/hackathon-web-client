...
const MyProfilePage = () => {
  ...
  const [relayHistory, setRelayHistory] = useState([
    ...
    { id: 3, date: "2024-12-30", distance: "8km", team: "Team Gamma", status: "failed", reason: "//실패 사유 알려주는 로직 추가" },
  ]);

  const handleClaimReward = (id) => {
    setRelayHistory((prev) =>
      prev.map((relay) =>
        relay.id === id ? { ...relay, rewardClaimed: true } : relay
      )
    );
  };
        ...
        {relayHistory.map((relay) => (
          <HistoryItem key={relay.id}>
            ...
            <Content>
              ...
              {relay.status === "failed" && (
                <p className="info">{relay.reason}</p>
              )}
            </Content>
          </HistoryItem>
        ))}
